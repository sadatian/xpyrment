# Implementation Plan: xpyrment Python Package (v1.6.1.2 Archive)

This is the archived implementation plan for **`xpyrment`**—a highly modular, phase-gated library designed to support the entire lifecycle of digital experimentation and classical Design of Experiments (DoE), capturing development history up to version `1.6.1.2`.

* For the previous, un-concentrated detail of blocks 1 to 60, see the [v0.1.0.0 Archive Implementation Plan](file:///c:/Users/Dan/projects/xpyrment/Gemini_Implementation_Plan_v0.1.0.0.md).
* For development history up to version 1.1.2.3, see the [v1.1.2.3 Archive Implementation Plan](file:///c:/Users/Dan/projects/xpyrment/Gemini_Implementation_Plan_v1.1.2.3.md).
...
- **Plots.py Code Coverage Diagnostics & Test Setup (Completed)**:
...
- **Sourcery Request Handler Quality Improvement (Completed)**:
  ...
  - Addressed all code review feedback for PR #34, introducing secure generic `send_server_error` handler to prevent leaking raw exception messages to clients, normalizing and validating feature/metric columns in OLS analysis to remove shape/KeyError bugs, and parametrizing SQL database schemes in unit testing.
  - Executed and verified the full 312-test suite with 100% green compliance.
- **MkDocs Build Warnings Resolved (Completed)**:
  - Added `TEST_MAPPING.md` under the root-level `nav` configuration in `mkdocs.yml` to resolve the unmapped page warning.
  - Prefix-typed the docstring inside `src/xpyrment/personalize/dragonnet.py` (specifically `estimate_effect`) with `r` to make it a raw string, eliminating the Python `SyntaxWarning` on invalid escape sequence `\h`.
  - Added `Any` to typing imports and annotated the `**kwargs` parameters with type `Any` for both `plot_interaction_heatmap` and `plot_interaction_effects` inside `src/xpyrment/interactions/plots.py`, clearing all `griffe` signature type warnings.
  - Verified the entire documentation build completes successfully with zero warnings or errors.
...
We have implemented Blocks 61-65 sequentially. For each block, we spun up:
...
### Phase 1: Block 61 — Dynamic SRM Shutoff Webhooks & Alert System (Completed & Verified ✅)
...
- **Verification Details**:
  - **Code File**: [src/xpyrment/run/monitor.py](file:///c:/Users/Dan/projects/xpyrment/src/xpyrment/run/monitor.py) (`WebhookAlertDispatcher`)
  - **Test Suite**: `tests/test_monitor_webhooks.py` (11/11 tests passed successfully)

### Phase 2: Block 62 — High-Performance Parquet & DuckDB Ingestion (Completed & Verified ✅)
...
- **Verification Details**:
  - **Code File**: [src/xpyrment/run/ingestion.py](file:///c:/Users/Dan/projects/xpyrment/src/xpyrment/run/ingestion.py) (`DuckDBIngester`)
  - **Test Suite**: `tests/test_duckdb_ingestion.py` (20/20 tests passed successfully)

### Phase 3: Block 63 — Deep Learning CATE Meta-Learners (Dragonnet / CausalML) (Completed & Verified ✅)
- **Developer & Advisor Subagents**: Build and optimize 3-headed joint representation neural models.
- **Verification Details**:
  - **Code File**: [src/xpyrment/personalize/dragonnet.py](file:///c:/Users/Dan/projects/xpyrment/src/xpyrment/personalize/dragonnet.py) (`DragonNet`)
  - **Test Suite**: `tests/test_personalize.py` (9/9 tests passed successfully)

### Phase 4: Block 64 — Autoregressive & Block-Bootstrap Covariance Structures (Completed & Verified ✅)
- **Developer & Advisor Subagents**: Implement Newey-West HAC standard errors and moving block bootstrap.
- **Verification Details**:
  - **Code File**: [src/xpyrment/analyze/inference/bootstrap.py](file:///c:/Users/Dan/projects/xpyrment/src/xpyrment/analyze/inference/bootstrap.py) (`run_block_bootstrap_ci`)
  - **Test Suite**: `tests/test_bootstrap_harden.py` (12/12 tests passed successfully)

### Phase 5: Block 65 — Interactive Live-Streaming Dashboard Web-UI (Completed & Verified ✅)
- **Developer & Advisor Subagents**: Design and compile standalone browser-based experiment dashboard.
- **Verification Details**:
  - **Code File**: [src/xpyrment/run/webui.py](file:///c:/Users/Dan/projects/xpyrment/src/xpyrment/run/webui.py) (`ExperimentDashboardServer`)
  - **Test Suite**: `tests/test_webui.py` (10/10 tests passed successfully)
...
- Maintain 100% green test passes across all suites.
