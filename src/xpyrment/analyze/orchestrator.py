...
class AnalysisResult:
    ...
    def summary(self, formatted: bool = True) -> pd.DataFrame:
        ...
        summary_df["95% CI (Rel)"] = np.where(ci_mask, "N/A", "[" + lower_str + ", " + upper_str + "]")

        p_val = df["p_value"]
        p_str = p_val.map("{:.4f}".format, na_action="ignore")
        sig_symbol = np.select([p_val < 0.001, p_val < 0.01, p_val < 0.05], ["***", "**", "*"], default="")
        summary_df["p-value"] = np.where(p_val.isna(), "N/A", p_str + sig_symbol)
        ...
        cuped_applied = df["cuped_applied"].fillna(False).astype(bool)
