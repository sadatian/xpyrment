...
class DuckDBIngester:
    ...
    def compute_covariate_balance(
        ...
        safe_treatment_col = self._escape_identifier(treatment_col)
        groups_df = self.query(
            f"SELECT DISTINCT {safe_treatment_col} FROM read_parquet(?) WHERE {safe_treatment_col} IS NOT NULL",
            [path_str]
        )
        ...
        if numeric_covs:
            ...
            for cov in numeric_covs:
                safe_cov = self._escape_identifier(cov)
                # the result keys need to remain compatible with standard unescaped names for the response dict
                # so we alias them normally without extra quotes for the key names (DuckDB will return this alias name)
                # Aliases can be double quoted so they are safe too. We'll extract them carefully or leave aliases unquoted if they are generated by us.
                # Actually it is safer to just double-quote the alias as well.
                alias_count = self._escape_identifier(f"count_{cov}")
                alias_mean = self._escape_identifier(f"mean_{cov}")
                alias_var = self._escape_identifier(f"var_{cov}")
                select_parts.append(f"COUNT({safe_cov}) as {alias_count}")
                select_parts.append(f"AVG({safe_cov}) as {alias_mean}")
                select_parts.append(f"VAR_SAMP({safe_cov}) as {alias_var}")
        ...
        for cov in categorical_covs:
            ...
            sql = f"""
                SELECT
                    ...
                    COUNT(*) as cnt
                FROM read_parquet(?)
                WHERE {safe_treatment_col} IN (?, ?) AND {safe_cov} IS NOT NULL
    ...
    def compute_welch_statistics(
        ...
        select_parts = []
        for m in metric_cols:
            safe_m = self._escape_identifier(m)
            alias_count = self._escape_identifier(f"count_{m}")
            alias_mean = self._escape_identifier(f"mean_{m}")
            alias_var = self._escape_identifier(f"var_{m}")
            select_parts.append(f"COUNT({safe_m}) as {alias_count}")
            select_parts.append(f"AVG({safe_m}) as {alias_mean}")
            select_parts.append(f"VAR_SAMP({safe_m}) as {alias_var}")
