# Implementation Plan: xpyrment Python Package (v1.1.2.3 Archive)

This is the archived implementation plan for **`xpyrment`**—a highly modular, phase-gated library designed to support the entire lifecycle of digital experimentation and classical Design of Experiments (DoE), capturing development history up to version `1.1.2.3`.
...
We converted custom callout/alert blocks to the native MkDocs `admonition` syntax to resolve rendering issues on the compiled documentation site and bumped the revision version to `1.1.0.1`.
...
### a) What was accomplished:
1. **Registering "AI Slop" License**: Updated the license field to `"AI Slop"` inside [pyproject.toml](file:///c:/Users/Dan/projects/xpyrment/pyproject.toml) and replaced the footer documentation inside [README.md](file:///c:/Users/Dan/projects/xpyrment/README.md) to state `"Distributed under the AI Slop License"`.
...
2. **Migrated Python Docstrings (Comprehensive)**: Located and migrated all occurrences of math sections and complex block math (`$$...$$`) across the entire codebase to use pink collapsible blocks, including:
   - `srm.py`: `??? mathbox "Mathematical Formulation"`
   - `novelty.py`: `??? mathbox "Mathematical Representation and Regression Detection"`
   - `balance.py`: `??? mathbox "Mathematical Representation"`
   - `power.py`: `??? mathbox "Mathematical Specifications"`
   - `hypothesis.py`: `??? mathbox "Mathematical Specifications"`
   - `transformations.py`: `??? mathbox "Mathematical Representation"` and `??? mathbox "Mathematical Context"`
   - `taxonomy.py`: `??? mathbox "Mathematical Background"` and `??? mathbox "Mathematical Representation"`
   - `guardrails.py`: `??? mathbox "Mathematical Representation"`
   - `taguchi.py`: `??? mathbox "Mathematical Specifications..."`
   - `simulation.py`: `??? mathbox "Mathematical and Generative Specifications"`
   - `stopping.py`: `??? mathbox "Mathematical Theory and Martingale Boundaries"`, `??? mathbox "Likelihood Ratio Formulation"`, and `??? mathbox "Mathematical Formulation"`
   - `monitor.py`: `??? mathbox "Temporal Binning and Accumulation Theory"`
   - `assignment.py`: `??? mathbox "First-Touch Attribution and Causal Ordering"`
   - `export.py`: `??? mathbox "Mathematical Relationship and CUPED Savings"`
   - `audit.py`: `??? mathbox "Cryptographic Verification and State-Chaining"`
   - `duration.py`: `??? mathbox "Mathematical Model"`
   - `significance.py`: `??? mathbox "Statistical Significance vs. Practical Significance"`
